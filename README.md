...
## 工厂模式与抽象工厂模式
### 概念
工厂模式是用来生产一批类的具体对象，使用者无须在通过new关键字来初始化对象，而是交由工厂来生产。

工厂模式将实例化的功能推迟到子类，父类（这里也包含接口）通常只提供创建实例的抽象方法。

抽象工厂与工厂模式的不同则是利用组合而不是继承的方式，通过将工厂类组合到具体的使用者中，达到可以更加自由组合的目的。

### 重要角色
#### 工厂模式
1. 父类：提供创建具体实例的接口，其他的具体功能可以均在父类中实现
2. 具体类：创建具体的类，实现父类的接口（创建具体实例）

#### 抽象工厂模式
1. 抽象工厂类：提供创建具体实例的接口
2. 具体的工厂类：实现抽象工厂（创建实例的具体接口），生产具体的类
3. 使用者：使用者通过将抽象工厂类组合到自己的类中，达到可以在运行过程中自由切换具体工厂的目的。

### UML图
#### 工厂模式
![MLJFkF.png](https://s2.ax1x.com/2019/11/24/MLJFkF.png)

#### 抽象工厂模式
![MLJtXt.png](https://s2.ax1x.com/2019/11/24/MLJtXt.png)